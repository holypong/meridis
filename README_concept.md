...
「なぜmeridis?」「どんな世界がひろがる?」を知りたい方はこちらをご覧ください。  
<BR>
...
**NVIDIA Newton（ニュートン）**  
NVIDIAが開発する人型ロボット（ヒューマノイド）向けの物理AIシミュレーション基盤。Isaac SimおよびIsaac Labと統合されており、GPU加速による大規模並列シミュレーションを活用してヒューマノイドロボットの運動制御・強化学習を効率的に実現する。2025年に発表されたNVIDIAのロボティクス戦略の中核をなすプラットフォームで、Sim2Real転移のための高精度物理モデルと大規模なロボット基盤モデルの訓練を目的として設計されている。meridis との連携により、Newtonで学習した制御ポリシーを実機ロボットへ容易に適用可能。
